   ...
   @if($result->count())
    <div class="content p-5 rounded bg-light d-flex align-self-center shadow">
        ...
        <div class="col deskripsi d-flex flex-col justify-content-center mt-0 fs-5 border-start ps-3">
            <span>Barang : {{ $result->nama }}</span>
            <span>Kategori : {{ $result->kategori->nama }}</span>
            <span>Lokasi ditemukan : {{ $result->lokasi }}</span>
            <span>Waktu ditemukan : {{ $result->waktu }}</span>
            <br>
            <p>Apakah ini punya anda ?</p>
            <a href="{{ route('verifikasi', $result->id) }}" class="bttn text-center mb-3">
                    <button class=" bttn">
                      Ya  
                    </button>
                </a>
